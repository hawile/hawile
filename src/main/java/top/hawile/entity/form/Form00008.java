package top.hawile.entity.form;

public class Form00008 {
    private String title;
    private String name;
    private int deptId;
    private String jobName;
    private int applyType1;
    private int applyType2;
    private int applyType3;
    private int applyType4;
    private int applyType5;
    private int applyType6;
    private int applyType7;
    private int applyType8;
    private String applyType9;
    private int accountType1;
    private int accountType2;
    private int accountType3;
    private int accountType4;
    private int accountType5;
    private int accountType6;
    private int accountType7;
    private int accountType8;
    private String accountType9;
    private String cause;
    private String changeContent;
    private String remark;

    public String getTitle() {
        return title;
    }

    public void setTitle(String title) {
        this.title = title;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public int getDeptId() {
        return deptId;
    }

    public void setDeptId(int deptId) {
        this.deptId = deptId;
    }

    public String getJobName() {
        return jobName;
    }

    public void setJobName(String jobName) {
        this.jobName = jobName;
    }

    public int getApplyType1() {
        return applyType1;
    }

    public void setApplyType1(int applyType1) {
        this.applyType1 = applyType1;
    }

    public int getApplyType2() {
        return applyType2;
    }

    public void setApplyType2(int applyType2) {
        this.applyType2 = applyType2;
    }

    public int getApplyType3() {
        return applyType3;
    }

    public void setApplyType3(int applyType3) {
        this.applyType3 = applyType3;
    }

    public int getApplyType4() {
        return applyType4;
    }

    public void setApplyType4(int applyType4) {
        this.applyType4 = applyType4;
    }

    public int getApplyType5() {
        return applyType5;
    }

    public void setApplyType5(int applyType5) {
        this.applyType5 = applyType5;
    }

    public int getApplyType6() {
        return applyType6;
    }

    public void setApplyType6(int applyType6) {
        this.applyType6 = applyType6;
    }

    public int getApplyType7() {
        return applyType7;
    }

    public void setApplyType7(int applyType7) {
        this.applyType7 = applyType7;
    }

    public int getApplyType8() {
        return applyType8;
    }

    public void setApplyType8(int applyType8) {
        this.applyType8 = applyType8;
    }

    public String getApplyType9() {
        return applyType9;
    }

    public void setApplyType9(String applyType9) {
        this.applyType9 = applyType9;
    }

    public int getAccountType1() {
        return accountType1;
    }

    public void setAccountType1(int accountType1) {
        this.accountType1 = accountType1;
    }

    public int getAccountType2() {
        return accountType2;
    }

    public void setAccountType2(int accountType2) {
        this.accountType2 = accountType2;
    }

    public int getAccountType3() {
        return accountType3;
    }

    public void setAccountType3(int accountType3) {
        this.accountType3 = accountType3;
    }

    public int getAccountType4() {
        return accountType4;
    }

    public void setAccountType4(int accountType4) {
        this.accountType4 = accountType4;
    }

    public int getAccountType5() {
        return accountType5;
    }

    public void setAccountType5(int accountType5) {
        this.accountType5 = accountType5;
    }

    public int getAccountType6() {
        return accountType6;
    }

    public void setAccountType6(int accountType6) {
        this.accountType6 = accountType6;
    }

    public int getAccountType7() {
        return accountType7;
    }

    public void setAccountType7(int accountType7) {
        this.accountType7 = accountType7;
    }

    public int getAccountType8() {
        return accountType8;
    }

    public void setAccountType8(int accountType8) {
        this.accountType8 = accountType8;
    }

    public String getAccountType9() {
        return accountType9;
    }

    public void setAccountType9(String accountType9) {
        this.accountType9 = accountType9;
    }

    public String getCause() {
        return cause;
    }

    public void setCause(String cause) {
        this.cause = cause;
    }

    public String getChangeContent() {
        return changeContent;
    }

    public void setChangeContent(String changeContent) {
        this.changeContent = changeContent;
    }

    public String getRemark() {
        return remark;
    }

    public void setRemark(String remark) {
        this.remark = remark;
    }
}
